...
split = ds.select(np.arange(int(len(ds)*(1/3)), int(len(ds)*(2/3))))

# Reset the index by adding an index column and then reordering
split = split.map(lambda x, idx: {"index": idx}, with_indices=True)

# Optionally, if you want to remove the old index column (if applicable)
split = split.remove_columns(["index"])

sari = load("sari")
file_path = './outputs/sari_scores_d2.txt'
pipe = pipeline(
...
for index, output in enumerate(pipe(KeyDataset(split, "chat_template"), max_new_tokens=512)):
