...
split = ds.select(np.arange(int(len(ds)*(2/3)), int(len(ds))))
...
sari = load("sari")
file_path = './outputs/sari_scores_d3.txt'
pipe = pipeline(
...
for index, output in enumerate(pipe(KeyDataset(split, "chat_template"), max_new_tokens=1024)):
